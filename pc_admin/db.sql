...
-- -----------------------------------------------------
-- Table `schoolconnect`.`mailreset`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `schoolconnect`.`mailreset` (
  `time` TIMESTAMP NOT NULL,
  `token` VARCHAR(512) NOT NULL,
  `people_id` VARCHAR(64) NOT NULL,
  `unix_hash` TEXT NOT NULL,
  `smb_hash` TEXT NOT NULL,
  PRIMARY KEY (`time`, `people_id`),
  UNIQUE INDEX `token_UNIQUE` (`token` ASC),
  INDEX `fk_mailreset_people1_idx` (`people_id` ASC),
  CONSTRAINT `fk_mailreset_people1`
    FOREIGN KEY (`people_id`)
    REFERENCES `schoolconnect`.`people` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
